...
function prepare() {
	...
	if (validate()) {
		...
		keys.status = document.getElementById("status").value;
		keys.drugClass = document.getElementById("drugClass").value;
		keys.route = document.getElementById("route").value;
		keys.dosage = document.getElementById("dosage").value;
		keys.schedule = document.getElementById("schedule").value;
		********/
...
function requestForResource(search) {
	...
	/********
	if(keys.status != "0") {
		query += "&search=@@." + keys.status;
	}
	
	if(keys.drugClass != "0") {
		query += "&search=@@." + keys.drugClass;
	}
	
	if(keys.route != "0") {
		query += "&search=@@." + keys.route;
	}
	
	if(keys.dosage != "0") {
		query += "&search=@@." + keys.dosage;
	}
	
	if(keys.schedule != "0") {
		query += "&search=@@." + keys.schedule;
	}
	********/
